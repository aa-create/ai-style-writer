  ...
  return (
    <section className="space-y-8">
      ...
      <div className="rounded-[32px] border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
        ...
        {activeTab === "text" ? <textarea rows={10} value={sampleText} onKeyDown={handleKeyDown} onChange={(event) => setSampleText(event.target.value)} placeholder="粘贴一篇你觉得写得好的范文……" className="w-full rounded-3xl border border-slate-200 bg-slate-50 px-5 py-4 text-sm leading-7 outline-none transition focus:border-slate-400" /> : <div className="space-y-4 rounded-3xl border border-dashed border-slate-200 bg-slate-50 p-5"><FileUpload isLoading={analyzing || learning || updating} fileNames={attachment ? [attachment.name] : []} onSelect={handleUpload} onRemove={() => setAttachment(null)} />{attachment ? <div className="rounded-2xl bg-white p-4 text-sm text-slate-600"><p className="font-medium text-slate-900">{attachment.name}</p><p className="mt-2 leading-7">{attachment.content.slice(0, 200)}...</p></div> : <p className="text-sm text-slate-400">上传后会显示文件名和前 200 字预览。</p>}</div>}
